...
|-------------------|------------|-----------------|
| Alpine containers | musl | `golang:1.26.3-alpine3.23` |
| Debian/Ubuntu containers | glibc | `golang:1.26.3-bookworm` |
| Ubuntu/Debian servers | glibc | `golang:1.26.3-bookworm` |
| RHEL/CentOS servers | glibc | Native build or glibc container |
| Kubernetes (Alpine) | musl | `golang:1.26.3-alpine3.23` |
| Kubernetes (Debian) | glibc | `golang:1.26.3-bookworm` |
...
Bifrost is built with **Go 1.26.3**. Your plugin **must** be compiled with the exact same Go version to ensure compatibility.

```bash
# Check your Go version
go version
# Should output: go version go1.26.3 ...

# If you need to install Go 1.26.3
...
**Solution:** Use the exact same Go version (Go 1.26.3) for both:
...
go version  # Should output: go version go1.26.3
...
```dockerfile
# Build plugin
FROM golang:1.26.3-alpine3.23 AS plugin-builder
WORKDIR /plugin
...
# Build Bifrost
FROM golang:1.26.3-alpine3.23 AS bifrost-builder
# ... (bifrost build steps)
